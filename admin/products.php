...
if ($_POST) {
	$errors = array();
 	...
 	if (!empty($_FILES)) {
 		...
 		$uploudPath = BASEURL.'images/products/'.$uploudName;
 		$dbpath = '/Tutorial/images/products/'.$uploudName;
 		if ($mimeType != 'image') {
 			$errors[] = 'The file must be image';
 		}
 		if (!in_array($fileExt, $allowed)) {
 			$errors[] = 'The file photo extension must be a png, jpg, or gif';
 		}
 	...
 	if (!empty($errors)) { 
 		...
 		if (!empty($_FILES)) {
 		move_uploaded_file($tmpLoc, $uploudPath);
 	}
...
?>
	...
	<form action="products.php?<?=((isset($_GET['edit']))?'edit='.$edit_id:'add=1'); ?>" method="POST" enctype="multipart/form-data">
		...
		<div class="form-group col-md-3">
			...
			<input type="text" class="form-control" name="sizes" id="sizes" id="sizes" value="<?= $sizes; ?>" readonly>
		</div>
		<div class="form-group col-md-6">
			<?php if($saved_image != ''): ?>
				<div class="saved-image">
					<img src="<?= $saved_image; ?>" alt="saved image"><br>
					<a href="products.php?delete_image=1&edit=<?=$edit_id; ?>" class="text-danger">Delete Image</a></div>
			<?php else: ?>
	...
	<!-- Modal -->
	<div class="modal fade" id="sizesModal" tabindex="-1" role="dialog" aria-labelelledby="sizesmyModalLabel" aria-hidden="true">
